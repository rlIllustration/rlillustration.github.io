...
formElements.form.addEventListener('submit', function(evt) {
	if(formElements.addendum1.checked || formElements.addendum2.value.length > 0) {
		formElements.errorContainer.innerHTML = '';
		evt.preventDefault();

		var msg = document.createElement('p');
		var msgText = document.createTextNode('An error occurred whilst submitting the form. Please refresh the page to try again');
